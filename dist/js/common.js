(function ($) {
	...
	$.getJSON('https://api.coinmarketcap.com/v1/ticker/', function (json) {
		...
		for (var i = 0; i < json.length; i++) {
			if (json[i].percent_change_24h > 0) {
				tr = $(
					'<tr><th scope="row"><div class="media"><div class="d-flex mr-2"><img src="https://s2.coinmarketcap.com/static/img/coins/32x32/' +
						json[i].rank +
						'.png" alt="" class="img-fluid" /></div><div class="media-body align-self-center">' +
						json[i].name +
						'.</div></div></th><td>' +
						json[i].price_usd +
						'.</td><td><i class="ion-arrow-graph-up-right pr-1 text-success"></i> ' +
						json[i].percent_change_24h +
						'.</td><td>' +
						json[i]['24h_volume_usd'] +
						'.</td><td>' +
						json[i].available_supply +
						'.</td></tr>'
				);
			} else {
				tr = $(
					'<tr><th scope="row"><div class="media"><div class="d-flex mr-2"><img src="https://s2.coinmarketcap.com/static/img/coins/32x32/' +
						...
						'.</div></div></th><td>' +
						json[i].price_usd +
						'.</td><td><i class="ion-arrow-graph-up-right pr-1 text-danger"></i> ' +
						json[i].percent_change_24h +
						'.</td><td>' +
						json[i]['24h_volume_usd'] +
						'.</td><td>' +
			...
			if (json[i].percent_change_24h > 0) {
				mr = $(
					'<div class="item text-center py-2"><a href="javascript:;"><span class="price-name text-uppercase text-white font-weight-bold">USD / ' +
						json[i].symbol +
						'</span><span class="price-value d-block text-muted">' +
						json[i].price_usd +
						' </span><span class="price-change text-success">+' +
						json[i].percent_change_24h +
						'% <i class="ion-arrow-graph-up-right"></i></span></a></div>'
				);
			} else {
				mr = $(
					'<div class="item text-center py-2"><a href="javascript:;"><span class="price-name text-uppercase text-white font-weight-bold">USD / ' +
						...
						'</span><span class="price-value d-block text-muted">' +
						json[i].price_usd +
						' </span><span class="price-change text-danger">-' +
						json[i].percent_change_24h +
						'% <i class="ion-arrow-graph-up-right"></i></span></a></div>'
				);
			}
	...
	tpj(document).ready(function () {
		if (tpj('#rev_slider_490_1').revolution == undefined) {
			...
			revapi490 = tpj('#rev_slider_490_1')
				.show()
				.revolution({
					sliderType: 'standard',
